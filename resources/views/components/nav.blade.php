<nav class="h-fit flex py-4 px-6 justify-between items-center bg-[#3494ce]">
    ...
    <div x-data="{ showButton: {{ Request::is('search') ? 'false' : 'true' }} }">
      ...
      @else
          <button  x-show="showButton" onclick="toggleLoginModal()" class="py-1 px-6 flex items-center font-medium text-xs hover:cursor-pointer bg-white fill-slate-400 rounded-md text-black hover:text-sky-500 hover:stroke-sky-500">
